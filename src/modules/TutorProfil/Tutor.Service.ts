...
const tutorAvailabilityService = async (data: any) => {
  ...
  let tutor_id = "";
  let tutor = await prisma.tutorProfile.findMany({
...
const tutorProfileExistService = async (id: string) => {
  let result = await prisma.tutorProfile.findFirstOrThrow({
    where: { userId: id },
  });
  console.log("_".repeat(50), "\n id:", id, "\n", result);
  return result;
...
const GettutorScheduleService = async (tutorId: string) => {
  // console.log("GettutorScheduleService",data)
...
export const tutorProfileService = {
  ...
  tutorProfileExistService,
  GettutorScheduleService,
};
